...
@Component({
  ...
  imports: [NgClass],
  template: `
    <div class="backdrop-blur-sm" [ngClass]="classMap.get(notification().type)?.mainContent" [class.notification-enter]="notification().state === 'enter'" [class.notification-leave]="notification().state === 'leave'" role="alert" tabindex="-1" aria-labelledby="hs-bordered-success-style-label">
          <div class="flex items-center">
            <div class="shrink-0">
              @switch (notification().type) {
                @case (types.SUCCESS) {
                  <span class="hidden md:inline-flex justify-center items-center size-8 rounded-full border-4 border-teal-100 bg-teal-200 text-teal-800 dark:border-teal-900 dark:bg-teal-800 dark:text-teal-400">
                ...
                @case (types.ERROR) {
                          <span class="hidden md:inline-flex justify-center items-center size-8 rounded-full border-4 border-red-100 bg-red-200 text-red-800 dark:border-red-500 dark:bg-red-800 dark:text-red-400">
                          <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
export class NotificationElement {
  ...
  classMap: Map<TypeNotification, NotificationElementClasses> = new Map<TypeNotification, NotificationElementClasses>([
    [TypeNotification.SUCCESS, {mainContent: 'bg-teal-50 border-t-2 border-teal-500 rounded-lg p-2 md:p-4 dark:bg-teal-800/80', textContent: 'text-teal-300'}],
    [TypeNotification.ERROR, {mainContent: 'bg-red-50 border-s-4 border-red-500 p-2 md:p-4 dark:bg-red-800/80', textContent: 'text-red-300'}]
  ])
